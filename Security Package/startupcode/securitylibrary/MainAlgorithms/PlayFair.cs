...
namespace SecurityLibrary
{
    public class PlayFair : ICryptographicTechnique<string, string>
    {
        ...
        public string Encrypt(string plainText, string key)
        {
            string cipherText = "";
            key = key.ToUpper();
            //remove duplicated letters in Key and push it in Matrix and pause with letter
            for (int i = 0; i < key.Length; i++)
            {
                for (int j = 0; j < key.Length; j++)
                {
            ...
            //we can expand and resize Capacity of string if we need to increase size size of plain text Ex : SA MM A in index -> SA MX MA
            StringBuilder edit = new StringBuilder(plainText.ToUpper()); 
            edit = edit.Replace("J", "I");
            int checkJ = 0;
